...
const restaurant = {};
const addItem = (request) => { restaurant.consumption.push(request); };
const createMenu = (objeto) => {  
  ...
  restaurant.pay = () => {
    let amountPay = 0;
    const order = restaurant.fetchMenu();
    for (const item of restaurant.consumption) {
      if (Object.keys(order.food).includes(item)) {
        amountPay += order.food[item];
      }
      if (Object.keys(order.drink).includes(item)) {
        amountPay += order.drink[item];
      }  
    }
    return amountPay;
  };
